...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="TORAX native DARTS X-ray explorer")
...
def main(argv=None) -> int:
    ...
    if args.no_3d:
        os.environ["TORAX_DISABLE_3D"] = "1"
    if "TORAX_DATA_ROOT" not in os.environ:
        project_root = Path(__file__).resolve().parents[3]
        project_var = project_root / "var"
        if not getattr(sys, "frozen", False) and project_var.exists():
            os.environ["TORAX_DATA_ROOT"] = str(project_var)
        else:
            os.environ["TORAX_DATA_ROOT"] = str(
                Path.home() / "Library" / "Application Support" / "TORAX"
            )
    ...
    try:
        ...
        print(
            "TORAX desktop dependencies are missing. Install with:\n"
    ...
    from jaxa_torax.desktop.screenshot import save_widget_screenshot
