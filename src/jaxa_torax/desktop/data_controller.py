...
def load_previews(
    ...
    with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="torax-preview") as pool:
        futures = {
...
def exact_energy(
    ...
    with ThreadPoolExecutor(
        max_workers=min(3, len(records)), thread_name_prefix="torax-exact"
    ) as pool:
...
def exact_all_events(records, region, bins: int, progress_callback=None) -> dict:
    ...
    with ThreadPoolExecutor(
        max_workers=min(2, len(records)), thread_name_prefix="torax-all-events"
    ) as pool:
        futures = {pool.submit(one, record): record for record in records}
        for completed, future in enumerate(as_completed(futures), start=1):
            record = futures[future]
