...
from jaxa_torax.desktop.theme import PLOT_TEXT, VIEWPORT
...
class ThreeDView(QWidget):
    ...
    def __init__(self):
        ...
        self.available = pv is not None and QtInteractor is not None and not os.getenv("TORAX_DISABLE_3D")
        self._plotter = None
    ...
    def set_top_image(self, product, region, mode="off", opacity=0.75, palette="gray",
        ...
        if not self.available or region is None:
            return
        try:
            from jaxa_torax.desktop.top_image import build_top_image
    ...
    def _picked(self, point):
        from jaxa_torax.desktop.picking import pick_event
