...
from jaxa_udon3.desktop.science_views import ImageProduct, rgb_from_channels
from jaxa_udon3.desktop.screenshot import save_widget_screenshot, show_screenshot_preview
...
class WindowActionsMixin:
    ...
    def reset_application(self):
        answer = QMessageBox.warning(
            self,
            "Reset UDON3 completely",
            "This removes downloaded cache, saved workspaces and logs, then restarts "
            "UDON3 as a new application. Exports and local source data are preserved.",
        ...
        started, _process_id = QProcess.startDetached(
            sys.executable,
            ["-m", "jaxa_udon3.desktop.app"],
            str(Path.cwd()),
        )
        if not started:
            QMessageBox.information(
                self, "Reset complete", "The app is clean. Close it and launch UDON3 again."
            )
        QApplication.quit()
    ...
    def export_preview(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export visible preview", "udon3_visible_events.csv",
    ...
    def save_screenshot(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Save viewer screenshot", "udon3_view.png", "PNG image (*.png)"
        )
        if not path:
            return
        try:
    ...
    def _show_help(self):
        QMessageBox.about(
            self,
            "About UDON3",
            "UDON3 — DARTS X-ray Event Explorer\n\n"
